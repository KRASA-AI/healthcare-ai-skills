...
- `ai_coding_tool` — whether the codes under review were suggested or auto-generated by an AI/autonomous-coding tool (e.g., a CodaMetrix/Fathom-class engine) or by an ambient scribe's coding assist, and if so which tool. When set, the review runs the §c AI-suggested-code provenance pass below and labels the workpaper accordingly. With no value, ask once in a single `[VERIFY: were any of these codes AI-suggested, and by what tool?]` flag and, absent an answer, proceed treating the codes as human-assigned while still applying the same over-coding rigor — never assume AI provenance and never assume human provenance silently.
...
3. Perform the following checks:
   ...
   **c. Over-Coding & Compliance Risks**
   ...
   - **AI-suggested-code provenance (when `ai_coding_tool` is set or provenance is disclosed).** An AI-suggested or autonomously-generated code is a *candidate*, not a verified code — the July 2026 GAO Science & Tech Spotlight on AI for medical notes and coding (GAO-26-109116) found that independent evidence of these tools' accuracy is limited and named accurate billing as an explicit oversight concern. Apply the same documentation-support test to an AI-suggested code that you would to a human-assigned one, and do not treat the tool's confidence, its "supported by the note" label, or its RAF/HCC suggestion as evidence. Two provenance-specific risks deserve a hard look: (1) an AI coder that mines the full chart can surface a code that is *historically true but not addressed at this encounter* (an HCC/MEAT-support gap, not a capture win) — confirm the condition was Monitored, Evaluated, Assessed, or Treated in the note being billed; and (2) when the note itself was produced by an ambient scribe, an AI coder reading an AI-drafted note can compound a documentation-intensity drift into a coding-intensity drift, so cross-check any elevated E/M level or added risk-adjusted diagnosis against the primary-source clinical facts rather than the drafted narrative alone. Flag AI-provenance items in the workpaper so a human coder's sign-off is recorded as the accountable review turn.
...
**Regulatory awareness (informational — does not change how a current claim is coded):** Medicare is beginning to build an explicit payment pathway for algorithm-driven clinical services. The CMS CY 2027 Physician Fee Schedule proposed rule (CMS-1848-P, released July 14, 2026; comment period closing September 14, 2026) proposes to pay for algorithm-only laboratory services under the PFS rather than the Clinical Laboratory Fee Schedule and, more broadly, signals CMS's intent to standardize how it pays for clinical software and AI. This is a *proposed* rule and not yet operative, so it does not affect code selection today. Its relevance to a coding review is forward-looking: where an encounter's billed service is delivered or materially augmented by an AI tool, note it so the practice can track whether a distinct payment category or status indicator ultimately attaches, and do not assume a separately billable "AI service" line exists until a final rule and an actual code/status indicator are published. Surface any such note as `[VERIFY: confirm the operative CY payment rule and any AI-service status indicator before billing]`.
