...
## Statutory Patient-Notification & Post-Visit Accuracy-Review Mandate (v1.5 addition — June 2026)

**Why this section exists.** The v1.4 patient-consent material framed per-encounter notification as a clinical-governance norm (NHS England) hardening into a litigation-risk surface (the April 2026 Sutter Health / MemorialCare class actions, which plead notification as a statutory *consent* requirement under CMIA/CIPA/Federal Wiretap law). On June 22, 2026 Rhode Island closed the remaining gap by enacting the first U.S. state statute that directly and affirmatively requires both (a) per-encounter patient notification of AI documentation use and (b) a clinician post-visit accuracy review of the AI-generated note. This converts the v1.4 per-encounter audit-header notification check from "best-practice elsewhere" into a *direct statutory obligation* in at least one state, and signals the model other states are likely to copy.

**Rhode Island H 7538 — Use of AI by Healthcare Providers Notification Act (signed June 22, 2026).** The Act requires all healthcare providers and healthcare facilities that employ AI to document an in-person or telehealth visit to (1) **notify the patient** that AI is being used for that purpose, and (2) **review the AI-generated documentation for accuracy after the visit.** The provider definition is broad — physicians, physician assistants, dentists, registered and licensed practical nurses, advanced-practice registered nurses, nursing assistants, and other state-licensed professionals — so the mandate reaches every ambient-scribe deployment in the state, not only physician encounters. The post-visit accuracy-review requirement is, in effect, a **statutory codification of the human-in-the-loop review turn** this audit has always treated as the integrity anchor: the "auto-accept" moment that Section 3 flags as the single largest source of drift is now, in Rhode Island, a point of legal non-compliance if no documented review occurred.

For audit purposes:

- **Per-encounter audit-header addition (Rhode Island, and as best-practice elsewhere).** Extend the v1.4 Section 1 audit-header notification field to capture two distinct facts for Rhode Island encounters: (1) whether a documented **patient-notification turn** occurred (the H 7538 notice), and (2) whether a documented **clinician post-visit accuracy review** occurred (the H 7538 review obligation, distinct from a bare e-signature). Missing patient notification on a Rhode Island ambient-documented encounter is `🟠 HIGH`. A signed note with **no evidence of a post-visit accuracy review turn** (i.e., an auto-accepted draft) on a Rhode Island encounter is `🟠 HIGH` by default and rises to `🔴 CRITICAL` where the unreviewed draft also carries a Section-3 fabrication-class finding (phantom diagnosis, drug-name mismatch, fabricated referral/order), because the statute's review requirement is precisely the control that would have caught it.
- **Governance / procurement addition.** For deployers operating in Rhode Island (or in any state that subsequently adopts an H 7538-style mandate), the vendor's product must support a documentable per-encounter notification event and a distinguishable clinician-review event in the audit log; a tool that records only a final e-signature without a separable review-turn artifact is a `🟠 HIGH` procurement gap because it cannot evidence statutory compliance. Feed this into the `healthcare-ai-governance-intake.md` intake under the data-management and risk-assessment pillars.
- **Relationship to the v1.4 case-law signal.** H 7538 and the Sutter/MemorialCare complaints point the same direction from two different legal mechanisms: the California complaints assert notification as an *interception-moment consent* harm under wiretap/medical-confidentiality law; the Rhode Island statute imposes notification *and* an affirmative post-visit review duty as front-end licensing-adjacent obligations. U.S. deployers should operationalize active per-encounter notification plus a documented review turn regardless of which framing binds first in their jurisdiction; the Rhode Island statute is the cleaner compliance anchor because it states the obligation affirmatively rather than leaving it to be inferred from a consent-harm theory.

**Cross-skill linkage.** The patient-notification artifact required by H 7538 is produced by the `patient-ai-use-disclosure-notice.md` skill (v1.1, which now carries the Rhode Island row); use that skill to draft the notice and this skill's Section 1 audit header to verify it was delivered and that a post-visit accuracy review was documented at each audited Rhode Island encounter.


## Behavioral-Health Consent Escalation & the Transcription Layer (v1.6 addition — July 2026)

**Why this section exists.** Two July 2026 signals push the audit's boundary in opposite directions — one *upstream* of the note (what happened before the microphone was turned on) and one *below* the note (what the speech-recognition layer heard before any language model summarized it). Neither changes how a drafted note is graded in Section 3, but both change what an auditor must have in scope before the grading is meaningful.

### A. Behavioral-health ambient capture now requires affirmative consent, not notification (Maine LD 2082, effective July 29, 2026)

The v1.4 and v1.5 additions traced a ladder: NHS England established per-encounter *notification* as a governance norm (March 2026); the Sutter / MemorialCare class actions pleaded it as an interception-moment *consent* harm (April 2026); Rhode Island H 7538 codified notification plus a post-visit accuracy review as an affirmative statutory duty (June 2026). Maine now adds the top rung. **Maine LD 2082 / HP 1397 (Public Law Ch. 687), approved April 13, 2026 and effective July 29, 2026**, regulates AI in the delivery of mental-health services: it confines a licensed mental-health professional's AI use to administrative and limited supplementary functions, bars routing therapeutic communications, treatment decisions, or independent patient interaction through AI — and requires the professional to obtain **patient consent before using an ambient-listening or other AI-powered recording tool**, framed as a written and revocable consent rather than a spoken heads-up.

The distinction matters operationally, because notification and consent produce different artifacts and different failure modes:

| | Notification duty (RI H 7538, NHS England) | Consent duty (ME LD 2082, behavioral health) |
|---|---|---|
| **Artifact** | A delivered notice; an audit-header field asserting it occurred | A signed, dated, revocable authorization retrievable per patient |
| **Default if silent** | Deployer may proceed having informed the patient | Deployer may **not** record |
| **Revocation** | Not contemplated | Must be honorable mid-course, and honored prospectively |
| **Audit question** | *Was the patient told?* | *Can you produce the consent, and was it still live at the time of capture?* |

For audit purposes, in Maine (and in any state that copies this pattern — treat this as the likely 2027 replication template, as Rhode Island's notification model was for 2026):

- **Per-encounter audit-header addition.** For any ambient-documented **behavioral-health** encounter in Maine, capture (1) whether a **retrievable, patient-specific written consent** predates the capture event, and (2) whether the consent was still in force at the time of capture (i.e., not revoked). A behavioral-health encounter recorded with **no retrievable consent artifact** is `🔴 CRITICAL` — not `🟠 HIGH` — because the defect is in the *lawfulness of the capture*, not in the quality of the note, and no downstream note correction cures it. Capture after a documented revocation is likewise `🔴 CRITICAL` and should halt the claim and trigger a privacy-incident review under the BAA's breach-notification clause.
- **Consent-revocation mechanics are a distinct procurement gap.** A vendor whose product can record a consent flag but cannot represent a **revocation** (or cannot suppress capture prospectively once revoked) is a `🟠 HIGH` procurement finding in any consent-regime state. Ask for this explicitly; a one-way "consent obtained: Y/N" checkbox is not sufficient evidence of compliance with a revocable-consent statute.
- **Scope boundary — do not over-generalize.** Maine's consent duty attaches to **mental-health services**, not to all ambient capture. A primary-care ambient encounter in Maine is governed by the general notification/consent perimeter, not by LD 2082. Auditors should not silently upgrade every Maine encounter to `🔴 CRITICAL`; state the service line in the audit header and apply the consent rule to the behavioral-health service line specifically. Where a general medical encounter surfaces substantial behavioral-health content, flag the ambiguity to counsel rather than resolving it inside the audit.
- **Cross-skill linkage.** `behavioral-health-ai-chatbot-compliance-review.md` v1.4 carries the Maine row for patient-facing behavioral-health surfaces; `patient-ai-use-disclosure-notice.md` produces the consent/disclosure language; `healthcare-ai-governance-intake.md` v1.5 carries the Maine row in its multi-state mapping and should capture the consent-revocation capability as a procurement-phase control.

### B. The transcription layer is a separate failure surface, and word error rate is the wrong metric for it

Every finding this skill has produced to date is graded against the *note*. But an ambient scribe is a two-stage pipeline — an automatic speech recognition (ASR) layer transcribes the encounter, and a language model summarizes the transcript into a note — and a fabrication-class finding in the note can originate in either stage. A drug name that was mis-heard at the ASR stage and then faithfully summarized is indistinguishable, in the finished note, from a drug name the summarizer invented. The remediation is entirely different in each case, and an audit that never looks below the note cannot tell them apart.

A vendor-published, openly-licensed 2026 agent-skills workflow for **clinical ASR evaluation** (observed July 2026) makes the underlying evaluation discipline explicit, and its central methodological claim is the part worth extracting: **aggregate word error rate is a misleading quality signal for clinical speech.** A transcription engine can post an excellent overall WER while systematically mangling precisely the tokens that carry clinical consequence — drug names, dosages and units, anatomical sites, negations, and specialty jargon — because those tokens are a vanishingly small share of total words. The discipline the workflow describes is to (1) curate a **specialty-specific list of clinically-salient terms** as a durable evaluation asset, (2) build a benchmark of clinical speech exercising those terms across accents, speaking rates, and acoustic conditions, and (3) score the engine on **term-level and entity-level error rates** against that list — a keyword/entity error rate — rather than on the undifferentiated word-level rate the vendor reports.

Translated into deployer-side audit terms — and stated without any dependency on the vendor's stack, models, or infrastructure, none of which this skill requires:

- **Audit-header addition.** Where the transcript is retained and retrievable, record whether the audit compared the note against the **transcript**, against the **structured chart**, or against **neither**. A note-only audit cannot attribute a fabrication-class finding to a stage and should say so rather than implying the summarizer is at fault.
- **New Section 3 sub-pattern — *transcription-origin error*.** When a Section 3 fabrication-class or drug-mismatch finding is raised **and** the transcript is available, check whether the erroneous term appears in the transcript. If the transcript already carries the wrong term, reclassify the finding as **transcription-origin** rather than summarizer-origin. Severity does not drop — a wrong drug is a wrong drug — but the remediation routes to the ASR layer (term-list tuning, vendor escalation, microphone/acoustic conditions, speaker accent coverage) rather than to summarizer prompting or HITL review depth. Mis-routed remediation is one of the quieter reasons ambient-scribe corrective actions fail to move the error rate.
- **Procurement-phase addition — do not accept an aggregate WER as an accuracy claim.** When a vendor offers a transcription-accuracy figure, ask what it is an error rate *of*. Request, in writing, the error rate on **clinically-salient terms** — the deployer's own high-risk medication list, the dosage-and-unit vocabulary, the specialty's anatomy and procedure terms — and the demographic and acoustic composition of the evaluation set (accents, non-native English speakers, speaking rates, ambient noise, and the languages the deployer's patient population actually speaks). A vendor who can produce only an aggregate WER, or who cannot state the evaluation set's composition, is a `🟠 HIGH` procurement gap. This is the transcription-layer analogue of the v1.3 Ontario procurement finding — accuracy is a *tool-and-deployment* property, not a category property — and it is the same evidence gap the worked example's Spanish-preferred subgroup exposes at the note layer.
- **Equity linkage.** The term-level framing is also the sharper equity instrument. Aggregate WER hides subgroup degradation; a term-level error rate broken out by speaker language, accent, and dialect makes it visible. Where a deployer serves a substantial non-native-English or non-English-preferred population, an accuracy claim that is not broken out by that population is not evidence about that population, and the Section 3 language-and-fidelity findings should say so explicitly.
- **What this section does not require.** This is an evaluation *frame*, not a demand that any deployer stand up a speech-benchmark pipeline. A community clinic auditing twenty notes a quarter should take exactly two things from it: compare against the transcript when one exists, and never accept an aggregate WER as a clinical-accuracy claim at procurement. The benchmark-construction machinery is a vendor-and-enterprise concern and is out of scope for this skill.

**Interaction with prior sections.** Part A extends the consent ladder built in v1.4 and v1.5 (notification → statutory notification + review → affirmative revocable consent in behavioral health) and is jurisdictionally bounded. Part B is jurisdiction-independent and *narrows attribution* rather than adding an obligation: it does not create new findings, it tells the auditor where an existing finding came from. Neither part relaxes any existing rating, and neither replaces the clinician's accountability for the finished record.
